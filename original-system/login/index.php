        ...
        if (!empty($account_password) && preg_match('/^(?=.*[a-zA-Z])(?=.*\d)[a-zA-Z\d]{8,16}$/', $account_password)) {
            try {
                ...
                if ($user && password_verify($account_password, $user['account_password'])) {
                    ...
                    $_SESSION['account']['workclass'] =(int) $user['account_workclass']; // 勤務区分
                    $_SESSION['auth'] = true; // 認証状態を保存

                    // ログイン時に、最終ログイン時間を更新する
                    $sql_update = 'UPDATE accounts SET last_login_time = :last_login_time WHERE account_id = :account_id';
                    $stmt_update = $pdoAccount->prepare($sql_update);
                    $stmt_update->bindValue(':last_login_time', date('Y-m-d H:i:s'), PDO::PARAM_STR);  // 現在時刻を設定
                    $stmt_update->bindValue(':account_id', $user['account_id'], PDO::PARAM_INT);  // ログインしたユーザーIDを設定
                    $stmt_update->execute();
