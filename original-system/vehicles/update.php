                    ...
                    <tr>
                        ...
                        <td colspan="2">
                            <select name="meter_inspection_year">
                                ...
                                <?php
                                    $currentYear = date("Y");
                                    $startYear = $currentYear - 1;  // 1年前から
                                    $endYear = $currentYear + 1;    // 1年後まで

                                    // POSTデータがあればそれを優先、なければデータベースからの値を使う
                                    $selected_meter_inspection_year = $_POST['meter_inspection_year'] ?? $meter_inspection_year;

                                    // 年の選択肢を生成
                                    echo generateJapaneseYearOptions($startYear, $endYear, $selected_meter_inspection_year);
                                ?>
                            </select>年
                            <select name="meter_inspection_month">
                                ...
                                <?php
                                    // POSTデータがあればそれを優先、なければデータベースからの値を使う
                                    $selected_meter_inspection_month = $_POST['meter_inspection_month'] ?? $meter_inspection_month;

                                    // 月の選択肢を生成
                                    echo generateMonthOptions($selected_meter_inspection_month);
                                ?>
                            </select>月
                        </td>
                    </tr>
                        ...
                        <td colspan="2">
                            <select name="lp_gas_year">
                                ...
                                <?php
                                    $currentYear = date("Y");
                                    $startYear = $currentYear - 1;  // 1年前から
                                    $endYear = $currentYear + 10;   // 10年後まで

                                    // POSTデータがあればそれを優先、なければデータベースからの値を使う
                                    $selected_lp_gas_year = $_POST['lp_gas_year'] ?? $lp_gas_year;

                                    // 年の選択肢を生成
                                    echo generateJapaneseYearOptions($startYear, $endYear, $selected_lp_gas_year);
                                ?>
                            </select>年
                            <select name="lp_gas_month">
                                ...
                                <?php
                                    // POSTデータがあればそれを優先、なければデータベースからの値を使う
                                    $selected_lp_gas_month = $_POST['lp_gas_month'] ?? $lp_gas_month;

                                    // 月の選択肢を生成
                                    echo generateMonthOptions($selected_lp_gas_month);
                                ?>
                            </select>月
                        </td>
                    </tr>
