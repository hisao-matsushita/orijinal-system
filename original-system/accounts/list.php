...
if (isset($_POST['submit'])) {
    try {
        ...
        $stmt_update->bindValue(':account_birthday', $_POST['account_birthday'], PDO::PARAM_STR);
        $stmt_update->bindValue(':account_license_expiration_date', $_POST['account_license_expiration_date'], PDO::PARAM_STR);
        ...
        $stmt_update->bindValue(':account_retirement_date', $_POST['account_retirement_date'], PDO::PARAM_STR);
        
        $stmt_update->execute();
        
        // 更新が完了した場合の処理
        $count = $stmt_update->rowCount();
        $message = urlencode("従業員情報を{$count}件更新しました。");
...
try {
    ...
    foreach ($accounts as $account) {
        ...
        // データをエスケープして$processedAccountsに追加
        $processedAccounts[] = [
            ...
            'registration_date' => isset($account['registration_date']) ? (new DateTime($account['registration_date']))->format('Y年m月d日') : '',
            'updated_at' => isset($account['updated_at']) ? (new DateTime($account['updated_at']))->format('Y年m月d日') : '',
